...
/**
 * @class Pinpoint.Controller.Session.Register
 */
Pinpoint.Controller.Base.extend('Pinpoint.Controller.Session.Register',
/** @Static */
{
	defaults : {}
},
...
initDisplay:function(inData){
	...
	var html=$.View('//pinpoint/controller/session/register/views/form.ejs',
		$.extend(inData, {
...
initDomElements:function(){
	...
	this.displayParameters.loginButton.domObj=$('#'+this.displayParameters.loginButton.divId);
			...
			this.displayParameters.loginButton.domObj.pinpoint_tools_ui_button2({
...
saveButtonHandler:function(control, parameter){
	...
	switch(control){
		...
		Pinpoint.Models.User.register(this.element.formParams(), this.callback('resetAfterSave'));
...
resetAfterSave:function(inData){
	...
	if (inData.status<1){
		$('#'+this.displayParameters.status.divId).html(errorString).removeClass('good').addClass('bad');
			...
			var html=$.View('//pinpoint/controller/session/register/views/confirmEmail.ejs',
		$.extend(inData, {
			displayParameters:this.displayParameters,
			viewHelper:this.viewHelper
		})
...
loginButtonHandler:function(control, parameter){
	...
	switch(control){
		case 'click':
			this.element.pinpoint_session_login();
		break;
		case 'setAccessFunction':
			if (!this[componentName]){this[componentName]={};}
			this[componentName].accessFunction=parameter;
...
return;
		var controlObj={
			calls:{
				schools:{
					ajaxFunction:Pinpoint.Models.School.getList,
					argData:{}
				}
			},
