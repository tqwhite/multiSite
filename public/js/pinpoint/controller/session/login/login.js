...
/**
 * @class Pinpoint.Controller.Session.Login
 */
Pinpoint.Controller.Base.extend('Pinpoint.Controller.Session.Login',
/** @Static */
{
	defaults : {}
},
...
initDisplay:function(inData){
	...
	var html=$.View('//pinpoint/controller/session/login/views/init.ejs',
		$.extend(inData, {
			displayParameters:this.displayParameters,
			viewHelper:this.viewHelper,
			formData:{
				newUsername:this.newUsername,
				userName:Pinpoint.Models.LocalStorage.getCookieData(Pinpoint.Models.LocalStorage.getConstant('loginCookieName')).data
			}
		})
		);
...
initDomElements:function(){
	...
	this.displayParameters.forgotButton.domObj=$('#'+this.displayParameters.forgotButton.divId);
			...
			this.displayParameters.saveButton.domObj.pinpoint_tools_ui_button2({
			...
			this.displayParameters.registerButton.domObj.pinpoint_tools_ui_button2({
...
/*
			this.displayParameters.forgotButton.domObj.pinpoint_tools_ui_button2({
				ready:{classs:'smallReady'},
				hover:{classs:'smallHover'},
				clicked:{classs:'smallActive'},
				unavailable:{classs:'smallUnavailable'},
...
saveButtonHandler:function(control, parameter){
	...
	switch(control){
		case 'click':
			Pinpoint.Models.Session.login(this.element.formParams(), this.callback('resetAfterSave'));
		break;
...
resetAfterSave:function(inData){
	...
	if (inData.status>0){
		Pinpoint.Models.Session.keep('user', inData.data);
		...
		Pinpoint.Models.LocalStorage.setCookie(Pinpoint.Models.LocalStorage.getConstant('loginCookieName'), inData.data.userName);
		this.element.html("<div style='margin-left:450px;margin-top:200px;' id='spinner'></div>").pinpoint_customer_dashboard();
...
registerButtonHandler:function(control, parameter){
	...
	switch(control){
		...
		this.element.pinpoint_session_register();
