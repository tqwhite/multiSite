 ...
 class Q_View_Helper_ProcessTemplateArray extends Zend_View_Helper_Abstract {
	 ...
	 public function processTemplateArray($args) {
		...
		\Q\Utils::validateProperties(array(
			...
			'source'=>__file__,
			'propertyList'=>array(
				array('name'=>'sourceData', 'requiredType'=>'array'),
				array('name'=>'itemTemplate', 'requiredType'=>'string'),
				
				array('name'=>'blockTemplate', 'importance'=>'optional'),
				array('name'=>'offset', 'importance'=>'optional'),
				array('name'=>'count', 'importance'=>'optional'),
				array('name'=>'transformations', 'importance'=>'optional'),
				array('name'=>'referenceData', 'importance'=>'optional')
			)));

		$sourceData=\Q\Utils::makeArrayNumericIndexed($args['sourceData']);
		$itemTemplate=$args['itemTemplate'];
		if (!isset($args['blockTemplate'])){$args['blockTemplate']='<!productList!>'; $blockTemplate=$args['blockTemplate'];} else{$blockTemplate=$args['blockTemplate'];}
		if (!isset($args['offset'])){$args['offset']=0; $offset=$args['offset'];} else{$offset=$args['offset'];}
		if (!isset($args['count'])){$args['count']=0; $count=$args['count'];} else{$count=$args['count'];}
		if (!isset($args['transformations'])){$args['transformations']=array(); $transformations=$args['transformations'];} else{$transformations=$args['transformations'];}
		if (!isset($args['referenceData'])){$args['referenceData']=array(); $referenceData=$args['referenceData'];} else{$referenceData=$args['referenceData'];}
