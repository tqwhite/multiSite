...
class Q_Model_Payment_Moneris{

public function __construct(){
	return "Note: this constructor exists so that Zend will autoload supplied classes in the file application/library/Q/Model/Payment/Moneris.php. This class is has no further use.";
}
}
...
class mpgGlobals
{
	...
	var $Globals=array(
        	        ...
        	        MONERIS_PORT =>'443',
                  	...
                  	API_VERSION  =>'US PHP Api v.3.1.0',
                  	CLIENT_TIMEOUT => '60'
                 	);
...
class mpgHttpsPost
{
 	...
 	var $mpgRequest;
 	var $mpgResponse;
 	var $debug=array(); //tqii so I could easily switch between servers

 	function getDebug(){
 		return $this->debug; //tqii
 	}

 	function mpgHttpsPost($storeid,$apitoken,$mpgRequestOBJ, $debug='')
 	{
 		if (gettype($debug)=='array'){
 			$this->debug=$debug;
 		}
  		...
  		if (isset($this->debug['host'])){
  			$gArray['MONERIS_HOST']=$this->debug['host']; //tqii
  		}
 ...
 // 		curl_setopt($ch, CURLOPT_CAINFO, '/Users/tqwhite/Documents/webdev/cmerdc/cmerdc.local/content/traxStore/_GLOBAL/COMPONENTS/curl-ca-bundle.crt'); //tqii
  		...
  		if (isset($this->debug)){
  			$this->debug['storeId']=$this->store_id;
			$this->debug['url']=$url;
			$this->debug['dataToSend']=$dataToSend;
  		...
  		if(!$response)
  		{
     			...
     			$response="<?xml version=\"1.0\"?><response><receipt>".
          			...
          			"<Message>Global Error Receipt-No Response From Processor</Message><TransAmount>null</TransAmount>".
...
class mpgRequest
{
 	...
 	var $txnTypes =array(
			...
			us_enc_track2_preauth => array('order_id','cust_id','amount','enc_track2','pos_code','device_type'),
			us_enc_track2_purchase => array('order_id','cust_id','amount','enc_track2','pos_code','device_type'),
			us_enc_track2_ind_refund => array('order_id','cust_id','amount','enc_track2','pos_code','device_type')
